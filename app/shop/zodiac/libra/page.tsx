  ...
  return (
    <>
      ...
      >
        ...
        <UniversalZodiacBanner signKey="libra" />

        {/* HORIZONTAL DROPDOWN NAVIGATION */}
        <ZodiacInfoNavigation zodiacSign="libra" />
        ...
        >
          <div className="max-w-7xl mx-auto p-4 pt-8 sm:p-8 sm:pt-12 w-full">
            {/* Libra Product Grid (replace with your libra products) */}
            {/* Example: <ReusableProductGrid products={libraProducts} columns={4} mobileColumns={2} gap="md" />; */}
          </div>
        </motion.div>
        ...
        {/* GLOBAL STYLES FOR REFINED LIBRA THEME */}
        <style jsx global>{`
          ...
          .libra-warm-animation {
            position: relative;
            overflow: hidden;
          }
          .libra-warm-animation::before {
            content: '';
            position: absolute;
            top: -50%;
            left: -50%;
          ...
          .venus-energy-pulse {
            animation: venusEnergy 4s ease-in-out infinite;
          }
          @keyframes venusEnergy {
            0%, 100% { 
